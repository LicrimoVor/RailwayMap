...
import { normalizeChunks, normalizeSegments, normalizeStations } from "./normalizers";
...
export async function fetchRailwayData(): Promise<RailwayData> {
  try {
    ...
    const cached = await readCachedValue<RailwayData>(INITIAL_RAILWAY_DATA_CACHE_KEY);
    if (cached) {
      return cached;
    }
    throw error;
...
export async function fetchStations(): Promise<StationFeatureCollection> {
  try {
    ...
    const cached = await readCachedValue<StationFeatureCollection>(STATIONS_CACHE_KEY);
    if (cached) {
      return cached;
    }
    throw error;
...
export async function fetchSegmentChunksForSection(
  ...
  const cached = await readCachedValue<RailwayChunkFeatureCollection>(cacheKey);
  if (cached) {
    return cached;
  }
