...
import type { RailwayChunkFeature, RailwayFeature } from "../../../../types/railway";
...
function handleChunkClick(
  ...
  const parentSegment =
    ...
    findFeatureById(latestStateRef.current.data.segments, chunkFeature.properties.segment_id);
  const chunk = { ...chunkFeature.properties, geometry: chunkFeature.geometry };
  const nextChunks = toggleChunk(latestStateRef.current.selectedChunks, chunk);
