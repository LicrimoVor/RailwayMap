...
import { electrificationLineColor } from "./layers";
...
export function updateLayerVisibility(
  ...
  setLayersVisibility(map, [STATION_LAYER_ID, STATION_LABEL_LAYER_ID], visibleLayers.stations);
  setLayerVisibility(map, EVENT_LAYER_ID, visibleLayers.events);
  setLayerVisibility(map, DEFECT_LAYER_ID, visibleLayers.defects);
  setRailwayColorMode(map, visibleLayers.electrification);
}

function setLayersVisibility(map: MapLibreMap, layerIds: string[], visible: boolean) {
...
function setRailwayColorMode(map: MapLibreMap, useElectrification: boolean) {
  if (map.getLayer(RAILWAY_LINE_LAYER_ID)) {
    map.setPaintProperty(
      RAILWAY_LINE_LAYER_ID,
      "line-color",
      useElectrification ? electrificationLineColor : "#c93535"
    );
  }
}
