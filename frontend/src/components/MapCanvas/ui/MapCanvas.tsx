...
import { emitViewportChange, emptyFeatureCollection, toggleChunk } from "../libs/mapHelpers";
	...
	useEffect(() => {
		...
		void resolveBaseMapStyle().then((style) => {
			...
			map.on("click", (event) => {
				...
				const chunkFeature = chunkFeatures[0] as unknown as
					| RailwayChunkFeature
					| undefined;
				if (chunkFeature) {
					const parentSegment =
				...
				const railwayFeature = features[0] as unknown as
					| RailwayFeature
					| undefined;
...
function updateBaseRailwaySource(map: MapLibreMap, data: RailwayData) {
	...
	stationSource.setData(
		data.stations as unknown as GeoJSON.FeatureCollection,
	);
}
...
function updateSelectedRailwaySource(
	map: MapLibreMap,
	selectedFeature: RailwayFeature | null,
) {
...
function updateSelectedChunksSource(
	map: MapLibreMap,
	selectedChunkFeatures: RailwayChunkFeature[],
) {
	const selectedChunksSource = map.getSource(
		SELECTED_CHUNKS_SOURCE_ID,
	) as GeoJSONSource;
...
function updateAdminSources(map: MapLibreMap, adminData: AdminMapData) {
	...
	eventSource.setData(
		adminData.events as unknown as GeoJSON.FeatureCollection,
	);

	const defectSource = map.getSource(DEFECT_SOURCE_ID) as GeoJSONSource;
	defectSource.setData(
		adminData.defects as unknown as GeoJSON.FeatureCollection,
	);
}
