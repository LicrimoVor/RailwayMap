...
export function formatLength(
	value: RailwaySegmentProperties["length_m"],
): string {
...
function findSectionFeature(
	...
	if (hasSectionOffsets(selection)) {
		const feature = segments.features.find(
			(item) =>
				...
				Number(item.properties.section_start_offset_m) ===
					Number(selection.section_start_offset_m) &&
				Number(item.properties.section_end_offset_m) ===
					Number(selection.section_end_offset_m),
		);
		if (feature) {
			return feature;
		}
	...
	return (
		segments.features.find(
			(item) =>
				String(item.properties.id) === String(selection.id) &&
				String(item.properties.section_id) ===
